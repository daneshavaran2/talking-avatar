...
Workflow روی GitHub ثبت شده و `state: active` است، ولی در اولین اجرا هر سه
Job **پیش از گرفتن Runner** شکست خوردند: صفر میلی‌ثانیه زمان محاسباتی، بدون
لاگ، و `runner_id: 0`. این الگو یعنی مشکل از فایل Workflow نیست بلکه از
سطح حساب است — سهمیهٔ GitHub Actions، سقف هزینه، یا غیرفعال بودن Actions
برای حساب.

اگر با همین وضعیت روبه‌رو شدید، تنظیمات صورتحساب حساب را بررسی کنید
(<https://github.com/settings/billing>). تا وقتی Runner تخصیص داده نشود،
هیچ Workflowای — هرقدر هم درست — اجرا نمی‌شود.

تا آن زمان، `npm run verify` و مجموعه‌های یکپارچه به‌صورت محلی همان
پوشش را می‌دهند.
