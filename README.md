...
`scripts/stub-services.mjs` هر سه پروتکل لازم را پیاده می‌کند (Ollama برای
مدل و تعبیه‌سازی، media-engine برای TTS) و WAV واقعی می‌سازد:

```bash
node scripts/stub-services.mjs   # پورت ۱۱۵۰۰
```
...
با این پیکربندی کد واقعی `OllamaProvider` و `MediaEngineTtsProvider` اجرا
می‌شود — چیزی شبیه‌سازی نشده جز خود سرویس بیرونی.
