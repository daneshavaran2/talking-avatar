...
#### ⚠️ وضعیت فعلی: Runner تخصیص داده نمی‌شود

Workflow روی GitHub ثبت شده و `state: active` است، ولی **هیچ Jobای اجرا
نمی‌شود**. نشانه‌ها در هر سه اجرا یکسان بود:

| نشانه | مقدار |
|---|---|
| `runner_id` / `runner_name` | `0` / خالی — هرگز Runner تخصیص داده نشد |
| زمان محاسباتی (`billable`) | **صفر میلی‌ثانیه** |
| مدت تا شکست | ~۳ ثانیه |
| لاگ | وجود ندارد (۴۰۴) |
| Annotation یا پیام خطا | خالی |

برای اینکه مطمئن شویم ایراد از این Workflow نیست، یک Workflow کمینه با
**یک Job و تنها یک دستور `echo`** — بدون Service، بدون Cache، بدون Action
بیرونی — ساخته و اجرا شد. نتیجه **دقیقاً همان**: صفر میلی‌ثانیه، بدون
Runner، شکست در سه ثانیه.

یک `echo` نمی‌تواند به دلیل محتوای خودش شکست بخورد. پس مسئله در فایل
Workflow نیست؛ GitHub برای این مخزن/حساب اصلاً منبع محاسباتی اختصاص
نمی‌دهد. جاهایی که باید بررسی شوند:

1. **صورتحساب و سقف هزینه** — <https://github.com/settings/billing>
   (بخش Actions؛ سقف هزینهٔ صفر یا اتمام سهمیه شایع‌ترین دلیل است)
2. **فعال بودن Actions برای مخزن** — Settings → Actions → General
3. **محدودیت‌های سطح حساب** — اگر حساب پرچم‌خورده باشد Actions مسدود می‌شود

تا وقتی این حل نشود، هیچ Workflowای اجرا نخواهد شد. پوشش آزمون در این
فاصله کاملاً از راه محلی تأمین می‌شود: `npm run verify` به‌علاوهٔ
مجموعه‌های یکپارچه که در بخش بالا فهرست شده‌اند.
